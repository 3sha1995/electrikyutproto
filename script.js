/* ======================================================
  ...
  function renderDailyChart(period){
    ...
    const W = 320, H = 150, padTop = 10, padBottom = 22, padSide = 6;
    ...
    if (caption) caption.textContent = `Average: ${data.baseline.toFixed(1)} kWh — bars above the dashed line ran over average`;
  }
  ...
  function renderDonutChart(){
    ...
    const segments = Array.from(document.querySelectorAll('.legend-row')).map(row => ({
  ...
  function renderHourlyAreaChart(){
    ...
    const values = [0.6,0.5,0.4,0.4,0.5,0.7,1.0,1.3,1.1,1.0,1.1,1.3,1.4,1.2,1.1,1.2,1.4,1.8,2.4,2.6,2.2,1.6,1.0,0.7];
    const W = 320, H = 130, padTop = 12, padBottom = 18, padSide = 4;
    const chartH = H - padTop - padBottom;
  ...
  function openPairingModal(){
    ...
    document.querySelectorAll('.found-device-row').forEach(r => r.classList.remove('selected'));
  ...
  function goToPairingStep(n){
    ...
    document.getElementById('pairingProgressFill').style.width = `${(n / 5) * 100}%`;
  ...
  function selectFoundDevice(row){
    ...
    pairingState.selectedDevice = { name: row.dataset.name, mac: row.dataset.mac };
  ...
  function resetConnectChecklist(){
    document.querySelectorAll('.connect-item').forEach(el => el.classList.remove('done', 'active'));
  ...
  function startConnecting(){
    if (pairingState.connectDone) {
      ...
      document.querySelectorAll('.connect-item').forEach(el => el.classList.add('done'));
  ...
  function fillPairingSummary(){
    ...
    document.getElementById('summaryDeviceName').textContent = pairingState.label || dev.name;
